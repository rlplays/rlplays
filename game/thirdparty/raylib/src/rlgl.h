...
void rlglInit(int width, int height)
{
    ...
    if (RLGL.State.defaultTextureId != 0) TRACELOG(RL_LOG_DEBUG, "TEXTURE: [ID %i] Default texture loaded successfully", RLGL.State.defaultTextureId);
...
unsigned int rlLoadTexture(const void *data, int width, int height, int format, int mipmapCount)
{
    ...
    if (id > 0) TRACELOG(RL_LOG_DEBUG, "TEXTURE: [ID %i] Texture loaded successfully (%ix%i | %s | %i mipmaps)", id, width, height, rlGetPixelFormatName(format), mipmapCount);
...
unsigned int rlLoadTextureDepth(int width, int height, bool useRenderBuffer)
{
    ...
    if (!useRenderBuffer && RLGL.ExtSupported.texDepth)
    {
        ...
        TRACELOG(RL_LOG_DEBUG, "TEXTURE: Depth texture loaded successfully");
    }
    else
    {
        ...
        TRACELOG(RL_LOG_DEBUG, "TEXTURE: [ID %i] Depth renderbuffer loaded successfully (%i bits)", id, (RLGL.ExtSupported.maxDepthBits >= 24)? RLGL.ExtSupported.maxDepthBits : 16);
    }
#endif
