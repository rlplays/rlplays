...
Image LoadImageFromMemory(const char *fileType, const unsigned char *fileData, int dataSize)
{
    ...
    if (image.data != NULL) TRACELOG(LOG_DEBUG, "IMAGE: Data loaded successfully (%ix%i | %s | %i mipmaps)", image.width, image.height, rlGetPixelFormatName(image.format), image.mipmaps);
...
void UnloadTexture(Texture2D texture)
{
    if (texture.id > 0)
    {
        ...
        TRACELOG(LOG_DEBUG, "TEXTURE: [ID %i] Unloaded texture data from VRAM (GPU)", texture.id);
    }
}
...
void DrawTexturePro(Texture2D texture, Rectangle source, Rectangle dest, Vector2 origin, float rotation, Color tint)
{
    ...
    if (texture.id > 0)
    {
        ...
        if (dest.width < 0) { flipX = true; dest.width *= -1; }
