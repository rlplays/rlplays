...
Font LoadFont(const char *fileName)
{
    ...
    if (isGpuReady)
    {
        ...
        else
        {
            ...
            TRACELOG(LOG_DEBUG, "FONT: Data loaded successfully (%i pixel size | %i glyphs)", font.baseSize, font.glyphCount);
        }
    }
            ...
            for (int i = 0; i < codepointCount; i++)
            {
                ...
                if (index > 0)
                {
                    ...
                    if (chars[i].image.data != NULL)    // Glyph data has been found in the font
                    {
                        ...
                        if (chh > fontSize) TRACELOG(LOG_DEBUG, "FONT: Character [0x%08x] size is bigger than expected font size", ch);
