...
Wave LoadWaveFromMemory(const char *fileType, const unsigned char *fileData, int dataSize)
{
    ...
    TRACELOG(LOG_DEBUG, "WAVE: Data loaded successfully (%i Hz, %i bit, %i channels)", wave.sampleRate, wave.sampleSize, wave.channels);
...
Music LoadMusicStream(const char *fileName)
{
    ...
    else
    {
        ...
        TRACELOG(LOG_DEBUG, "FILEIO: [%s] Music file loaded successfully", fileName);
        TRACELOG(LOG_DEBUG, "    > Sample rate:   %i Hz", music.stream.sampleRate);
        TRACELOG(LOG_DEBUG, "    > Sample size:   %i bits", music.stream.sampleSize);
        TRACELOG(LOG_DEBUG, "    > Channels:      %i (%s)", music.stream.channels, (music.stream.channels == 1)? "Mono" : (music.stream.channels == 2)? "Stereo" : "Multi");
        TRACELOG(LOG_DEBUG, "    > Total frames:  %i", music.frameCount);
    }
